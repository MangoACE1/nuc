...
- 连续确认窗口：`2`
- 空气阻力：`drag_beta_m_inv = 0.005 m⁻¹`（由实测数据标定，二次阻力，用于 EKF 与弹道预测）
...
- 球实际位置：`pose_measured_*`（最近一次接收的原始位置测量）
...
进入 `DONE`/`ERROR` 后对象订阅销毁，原始测量列会清空（空值），不会再用过期数据制造假残差；`actual_capture_*` 保留最后一次投掷的最终结果。写 CSV 失败只会在日志中报错并禁用记录，不会影响预测进程。
